...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className='bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-4 text-center shadow-lg transition-all duration-300 flex flex-col hover:-translate-y-1.5 hover:shadow-teal-600/10 hover:border-teal-600/30 h-full'>
      ...
      <div className='flex-grow flex flex-col justify-between'>
        ...
        <div className='mt-4'>
          <p className='font-bold text-teal-600 text-2xl mb-4'>${product.price ? product.price.toFixed(2) : '0.00'}</p>

          <div className='grid grid-cols-2 gap-2 items-center justify-between'>
            <input
              type="number"
              className='bg-white border border-teal-600 text-gray-800 text-center rounded-lg py-2 focus:outline-none focus:border-teal-700 w-full transition-colors duration-300 font-semibold'
              value={quantity}
              onChange={(e) => setQuantity(e.target.value)}
              min="1"
            />
            <button onClick={handleAddToCart} className='bg-teal-600 hover:bg-teal-700 text-gray-900 font-bold py-2 px-3 rounded-lg transition-all duration-300 shadow-lg shadow-teal-600/20 w-full'>
              Agregar
            </button>
          </div>
        </div>
