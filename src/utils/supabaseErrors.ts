 ...
 * @param context - Contexto opcional para personalizar el mensaje (ej: 'producto', 'imagen')
 * @returns Mensaje amigable para mostrar al usuario
 */
export function handleSupabaseError(error: SupabaseError | null, context = ''): string {
  if (!error) return '';
 ...
 *
 * @param supabasePromise - Promesa de Supabase (ej: supabase.from('x').select('*'))
 * @param context - Contexto para el mensaje de error
 * @returns Promise con data, error y userMessage
 */
export async function safeSupabaseCall<T>(supabasePromise: Promise<{ data: T | null; error: SupabaseError | null }>, context = ''): Promise<SupabaseCallResult<T>> {
  try {
    ...
    return {
      data: null,
      error: err as SupabaseError,
      userMessage: handleSupabaseError(err as SupabaseError, context),
    };
  }
}
 ...
 */
export function isNotFoundError(error: SupabaseError | null): boolean {
 ...
 */
export function isDuplicateError(error: SupabaseError | null): boolean {
 ...
 */
export function isPermissionError(error: SupabaseError | null): boolean {
