...
const BulkPriceUpdate = () => {
    ...
    return (
        <div className="w-full border-collapse shadow-xl bg-white p-4 md:p-8 rounded-xl font-outfit flex flex-col flex-1 min-h-0 overflow-y-auto custom-scrollbar">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end border-b border-gray-300 pb-3 mb-3 gap-3">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-xs md:text-sm italic hidden md:block">Gestiona múltiples precios con precisión y rapidez.</p>
                </div>
                {updateMessage && (
            ...
            <div className="bg-white py-2 flex flex-col md:flex-row flex-wrap gap-2 md:gap-4 items-stretch md:items-center">
                ...
                <div className="flex gap-2 items-center flex-1 md:flex-none">
                    <select
                        className="flex-1 md:flex-none p-2 border rounded-lg bg-gray-50 outline-none h-[38px] text-xs"
                    ...
                    <div className="text-[10px] md:text-sm text-gray-500 font-bold bg-gray-50 px-3 py-2 rounded-lg border border-gray-100 whitespace-nowrap">
                        {selectedProducts.size} selec.
                    </div>
                </div>
            </div>

            {/* Panel de Acción - Ahora arriba y compacto */}
            <div className="bg-slate-50 p-2 md:p-6 border rounded-xl mb-4 md:mb-6 shadow-sm mt-3">
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2">
                    <div className="flex items-center gap-1.5 w-full md:w-auto">
                        <select
                            className="w-[100px] md:w-auto bg-white border px-2 rounded-lg font-bold text-gray-700 outline-none text-xs h-[38px] flex-shrink-0 shadow-sm"
                            value={operation}
                            onChange={(e) => setOperation(e.target.value)}
                        >
                            <option value="increase">Aum (+)</option>
                            <option value="decrease">Dis (-)</option>
                            <option value="visibility">👀 Visibilidad</option>
                        </select>

                        <div className="flex-1 md:flex-none flex items-center gap-1.5 min-w-0">
                            {operation === 'visibility' ? (
                                <div className="flex bg-white border p-0.5 rounded-lg w-full md:w-48 h-[38px] shadow-sm">
                                    <button
                                        className={`flex-1 rounded-md text-[10px] font-bold transition-all flex items-center justify-center ${allHidden
                                    ...
                                    <button
                                        className={`flex-1 rounded-md text-[10px] font-bold transition-all flex items-center justify-center ${allVisible
                                            ? 'text-gray-300 cursor-not-allowed'
                                ...
                                <div className="flex items-center gap-1.5 w-full">
                                    <div className="flex bg-white border p-0.5 rounded-lg flex-shrink-0 h-[38px] shadow-sm items-center">
                                        <button
                                            className={`w-8 h-full rounded-md text-xs font-bold transition-all ${valueType === 'percentage' ? 'bg-blue-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
                                            onClick={() => setValueType('percentage')}
                                        >
                                            %
                                        </button>
                                        <button
                                            className={`w-8 h-full rounded-md text-xs font-bold transition-all ${valueType === 'fixed' ? 'bg-blue-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
                                    ...
                                    <input
                                        type="number"
                                        className="bg-white border px-2 rounded-lg flex-1 md:w-24 font-black text-center outline-none ring-2 ring-transparent focus:ring-blue-400 transition-all text-base h-[38px] min-w-0 shadow-sm"
                                        value={amount}
                    ...
                    <button
                        ...
                        className={`hidden md:flex px-6 rounded-lg font-black text-white shadow-md transition-all transform active:scale-95 text-sm h-[38px] items-center justify-center ${selectedProducts.size === 0 || (operation !== 'visibility' && amount <= 0) || isUpdating
                    ...
                    >
                        {isUpdating ? '...' :
                            operation === 'visibility'
                                ...
                                : `Confirmar Actualización (${selectedProducts.size})`
                        }
                    </button>

                    {/* Accordion: Opciones Avanzadas - Compacto */}
                    <div className="flex md:flex-1 items-center justify-between md:justify-end gap-3 mt-1 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-200">
                        <button
                            onClick={() => setShowAdvancedOptions(!showAdvancedOptions)}
                            className="flex items-center gap-1.5 text-[10px] text-gray-400 hover:text-gray-600 font-bold uppercase transition-colors"
                        >
                        ...
                        {showAdvancedOptions && (
                            <div className="flex items-center gap-3 bg-white/50 px-2 py-1 rounded-md border border-gray-100 shadow-sm transition-all animate-in fade-in zoom-in duration-200">
                                {operation !== 'visibility' && (
                                    <div className="flex items-center gap-1.5">
                                        <label className="text-[9px] text-gray-400 font-bold uppercase">Redon:</label>
                                        <select
                                            className="bg-white border p-0.5 rounded-sm text-[10px] outline-none"
                                            value={rounding}
                                            onChange={(e) => setRounding(e.target.value)}
                                        >
                                            <option value="none">Off</option>
                                            <option value="to10">10s</option>
                                            <option value="to100">100s</option>
                ...
                <div className='hidden md:flex flex-col flex-1 mt-5 overflow-hidden border border-gray-200 rounded-lg bg-white'>
            ...
            {/* FAB - Botón Flotante para Confirmar (Móvil) */}
            {selectedProducts.size > 0 && (
                <div className="fixed bottom-6 left-0 right-0 flex justify-center px-4 md:hidden z-[100] animate-in slide-in-from-bottom-5 duration-300 pointer-events-none">
                    <button
                        onClick={operation === 'visibility' ? handleVisibilityUpdate : handleUpdate}
                        disabled={
                            isUpdating ||
                        ...
                        className={`pointer-events-auto flex items-center gap-2 px-8 py-3.5 rounded-full font-black text-white shadow-[0_10px_40px_rgba(0,0,0,0.3)] transition-all transform active:scale-95 text-base border-2 border-white/30 ${isUpdating || (operation !== 'visibility' && amount <= 0)
                            ? 'bg-gray-400 opacity-80'
                            : operation === 'visibility'
                                ? visibilityAction === 'hide' ? 'bg-orange-500' : 'bg-green-600'
                                : 'bg-blue-600'
                            }`}
                    >
                        {isUpdating ? 'Actualizando...' : `Confirmar (${selectedProducts.size})`}
                    </button>
                </div>
            )}
        </div>
