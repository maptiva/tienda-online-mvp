...
import { statsService, ShopStatsSummary } from '../services/statsService';
...
const StatsPage: React.FC = () => {
    ...
    if (storeLoading) return <LazyLoadFallback />;
        ...
        <div className='w-full shadow-lg border border-gray-200 bg-white p-0 rounded-2xl flex flex-col flex-1 min-h-0 overflow-hidden'>
            <div className='flex-1 flex flex-col overflow-y-auto custom-scrollbar p-4 md:p-8'>
                <div className="flex justify-between items-center border-b border-gray-200 pb-4 mb-8">
                    <div>
                        <h1 className='text-2xl md:text-3xl font-bold text-gray-800'>Estadísticas e <span className="text-[#5FAFB8]">Insights</span></h1>
                        <p className="text-gray-500 text-sm mt-1">Monitorea el rendimiento de tu tienda en tiempo real.</p>
                    </div>
                    <button 
                        onClick={fetchStats}
                        disabled={statsLoading}
                        className="bg-gray-100 hover:bg-gray-200 p-2 rounded-lg transition-colors text-gray-600 flex items-center gap-2"
