...
interface InventoryBackup {
  id: number | string;
  quantity: number;
  reserved_quantity: number;
}

interface ProcessPublicCartSaleResult {
  success: boolean;
  results: Array<{ product_id: number; success: boolean }> | string;
}
...
async function run() {
  ...
  const backups: Record<number, InventoryBackup | null> = {};
  ...
  try {
    for (const t of testCases) {
      ...
      const rpcData = data as unknown as ProcessPublicCartSaleResult;

      if (t.expectSuccess) {
        if (!rpcData.success) throw new Error(`Expected success for product ${t.product_id} but got ${JSON.stringify(rpcData)}`);

        // Results may be JSON object with 'results' as array/JSON string
        let resArray: Array<{ product_id: number; success: boolean }> = [];
        if (!Array.isArray(rpcData.results)) {
          try { resArray = JSON.parse(rpcData.results as string); } catch { resArray = []; }
        } else {
          resArray = rpcData.results;
        }
        ...
        if (rpcData.success) throw new Error(`Expected failure for product ${t.product_id} but rpc success true`);
